...
export default function EditContractAction({contractId, currentContractData}: {contractId: number, currentContractData: EditContract}){
    ...
    return(
        <>
            <button className="bg-gray-500 hover:bg-gray-700 rounded px-3 py-2 text-sm text-white cursor-pointer" onClick={() => {setOpenEditContract(true)}}>{t("contracts.editContract")}</button>

            <Modal open={openEditContract} onClose={()=>{setOpenEditContract(false)}}>
                <form action="">
                    <h1 className="text-heading md:text-2xl lg:text-2xl mb-3">{t("contracts.editContract")}</h1>
                    
                    <h2>{t("contracts.status")}</h2>
                    <label className="relative inline-flex items-center cursor-pointer mb-5">
                        ...
                        <span className="ml-3 text-sm font-medium text-gray-900">
                            {editedContract.isActive ? t("common.active") : t("common.inactive")}
                        </span>
                    </label>
